...
def create_dbt_source_config(template_file, outfile, as_version="6.1.1", bigscape_cutoff="0.30"):
    """
    ...
    with open(source_yml, "r") as f:
        ...
        source_template = yaml.safe_load(raw_template.render(as_version=as_version, bigscape_cutoff=bigscape_cutoff))
    ...
    for num_s, s in enumerate(sources):
        ...
        for num_t, t in enumerate(s['tables']):
            ...
            table = [i for i in (dbt_dir / location).parent.glob(f"*{name}*.{table_extension}")]
            if len(table) == 1:
                ...
                assert len(template) == 1, f"Error finding template for {s['name']}.{name}: {template}"
                table = template
                logging.debug(f"Found table {s['name']}.{name}: {table[0]}")
                logging.debug(f"Replacing source value: {location} --> {template_location}")
                source_template['sources'][num_s]['tables'][num_t]['meta']['external_location'] = template_location
            else:
